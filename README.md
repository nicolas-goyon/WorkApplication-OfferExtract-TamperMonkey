...
| Hellowork | `src/sites/hellowork/` | Scoped to the `#offer-panel` container: title/company from the header, location from the chip list, description from the always-open section body plus the collapsible `<details>` sections ("Le profil recherché", "Infos complémentaires", ...), which are opened first since their content is genuinely hidden until expanded. The photo gallery is excluded. Also registered as a template. |
| Choisir le service public | `src/sites/choisirleservicepublic/` | French public-sector job board (choisirleservicepublic.gouv.fr, DSFR markup), scoped to `main.main-content-single-offer`. Title/reference/employer/location and the details table (contract, salary, category, remote, ...) are read field by field from the hero strate; the offer body ("Vos missions en quelques mots", "Profil recherché") and the "Qui sommes-nous ?" blurb come from the two-column strates, whose "Afficher la suite" clamps hide the tail of every long text behind a real `hidden` attribute (`.text-clamp--hiddentext`) and are un-hidden first; the "À propos de l'offre" accordions are read as label/value pairs off `textContent` since their collapsed panels stay `visibility:hidden`. The page's JobPosting JSON-LD is malformed (bare-string employer, capital-D `Description`), so the DOM overrides do the real work. Similar-offer cards are excluded. Also registered as a template. |
